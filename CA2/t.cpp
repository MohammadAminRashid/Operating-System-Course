...
int main()
{
    ...
    while (1)
    {
        ...
        // cout<<redirect_in_np<<" "<<redirect_out_np<<endl;
        // exit(1);
        ...
        if (redirect_in_np)
        {
            int fd_np = open(NAMEPIPE, O_RDONLY);
            if (fd_np == -1)
            {
                perror("open np");
                continue;
            ...
            string line;
            char c;
            while (read(fd_np, &c, 1) == 1)
            {
                if (c == '\n')
                    break; // فقط تا اولین newline بخوان
                line.push_back(c);
            }
            close(fd_np);
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (pid > 0)
            {
                ...
                if (i == n - 1)
                {
                    ...
                    if (redirect_out_np)
                    {
                        int fd_np = open(NAMEPIPE, O_WRONLY);
                        string msg = out + "\n"; // اضافه کردن delimiter
                        write(fd_np, msg.c_str(), msg.size());
                        close(fd_np);
                    }
                ...
                write(fd[1], out.c_str(), out.size() + 1);
                close(fd[1]);
